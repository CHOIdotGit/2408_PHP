<template>
    <div class="form-box">
        ...
        <input v-model="userInfo.name" type="text" name="name" placeholder="닉네임" autocomplete="off">
        <div class="radio-box">
            <div>
            ...
            <div>
                <label for="female">기타</label>
                <input v-model="userInfo.gender" type="radio" name="gender" id="etc" value="2">
            </div>
        </div>
        <div>
            <h3>프로필 사진 선택</h3>
        ...
        <hr>
        <button @click="$store.dispatch('user/registration', userInfo)" class="btn btn-bk">회원가입</button>
        <button @click="$router.replace('login')" class="btn gr">취소</button>
    </div>
</template>
