...
-- 1-1. users(유저) Table
-- 	- pk, email(id), password, name, 가입일자, 수정일자, 탈되일자
CREATE TABLE users(
...
-- 1-2. boards(게시판) Table
-- 	- pk, 유저 pk, 게시판 타입, 제목, 내용, 이미지파일, 작성일자, 수정일자, 삭제일자
CREATE TABLE boards(
...
-- 1-3. boards_category(게시판 기본 정보)Table
-- 	- pk, 게시판 타입, 게시판 이름
